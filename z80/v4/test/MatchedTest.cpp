...
TEST_CASE("Opcode bit parsing") {
  SECTION("LD xx, IMM16") {
    constexpr auto matched = parse_opcode_bits("00pp0001");
  ...
  SECTION("LD r, r'") {
    constexpr auto matched = parse_opcode_bits("01yyyzzz");
  ...
  SECTION("Wholly fixed") {
    constexpr auto matched = parse_opcode_bits("11001001");
  ...
  SECTION("Wholly variable") {
    constexpr auto matched = parse_opcode_bits("nnnnnnnn");
  ...
  SECTION("Rejects bad patterns") {
    CHECK_THROWS(parse_opcode_bits("0101"));
    CHECK_THROWS(parse_opcode_bits("011011011"));
    CHECK_THROWS(parse_opcode_bits("00pp0p01"));
    CHECK_THROWS(parse_opcode_bits("abcde001"));
  }
  SECTION("Accepts the widest supported field count") {
    constexpr auto matched = parse_opcode_bits("wwxxyyzz");
    STATIC_CHECK(matched.num_slices == Matched::max_slices);
    STATIC_CHECK(matched.fixed_mask() == 0);
  }
}

constexpr auto ld_rr_imm16 = parse_opcode_bits("00pp0001");
