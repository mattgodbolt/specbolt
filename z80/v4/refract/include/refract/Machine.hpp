...
// indexed address and spends time.
//
// Where *operation* names are resolved is a separate question, answered by the palettes the target lists and by the
// members the machine publishes with `[[=refract::operation]]`; see Execute.hpp. Where *location* names are resolved is
// not a question the target answers at all: a location is a thing the machine can read, so the pool is its own `read`
// overloads. See `location_scopes` in Execute.hpp.
//
...
//
// A machine also needs `read` and `write` overloads for each kind of location it declares, but those cannot be written
// down here: how many there are, and what they take and return, is whatever the machine's own location scopes say. A
// row naming a location the machine cannot reach is diagnosed where it is spliced instead.
...
// The name of the member the scan for a machine's location scopes looks for. It is named here because that scan finds
// overloads by this spelling, and this is the file stating what a machine must provide: a public `read(E)` overload
// publishes every enumerator of `E` to descriptions. `write` needs no such constant: the generator spells it at the
// calls it splices.
...
// and the interpreter finds them. Checked as a constraint so that a target missing a piece is told at the point it is
