...
std::string upper(std::string_view input_v) {
...
Op match_op(Opcode opcode) {
  if (opcode.x == 0 && opcode.y == 0 && opcode.z == 0)
    return {"nop", {}};

  if (opcode.x == 0 && opcode.y == 1 && opcode.z == 0)
    return {"ex af, af'", {"regs_.ex(RegisterFile::R16::AF, RegisterFile::R16::AF_);"s}};

  if (opcode.x == 0 && opcode.z == 1 && opcode.q == 0) {
    return {std::format("ld {}, $nnnn", opcode.reg_set.rp[opcode.p]),
        {
            std::format("regs_.set(RegisterFile::R8::{}, read_immediate());", upper(opcode.reg_set.rp_low[opcode.p])),
            std::format("regs_.set(RegisterFile::R8::{}, read_immediate());", upper(opcode.reg_set.rp_high[opcode.p])),
        }};
  }
  ...
  if (opcode.x == 0 && opcode.y == 2 && opcode.z == 0)
    return {"djnz $d", {
                           ...
                           "const std::uint8_t new_b = regs_.get(RegisterFile::R8::B) - 1;",
                           "regs_.set(RegisterFile::R8::B, new_b);",
                           "if (new_b == 0) return;",
                           "pass_time(5);",
                           "branch(offset);",
                       }};
