...
void Memory::raw_write(const std::uint16_t address, const std::uint8_t byte) { memory_[address] = byte; }

void Memory::load(const std::filesystem::path &filename, const std::uint16_t address, const std::uint16_t size) {
  ...
  if ((static_cast<std::size_t>(address) + size) > memory_.size()) {
